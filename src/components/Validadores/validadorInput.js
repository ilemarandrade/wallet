...
const nombres=/^([A-Za-zÁÉÍÓÚñáéíóúÑ]{0}?[A-Za-zÁÉÍÓÚñáéíóúÑ\']+[\s])+([A-Za-zÁÉÍÓÚñáéíóúÑ]{0}?[A-Za-zÁÉÍÓÚñáéíóúÑ\'])+[\s]?([A-Za-zÁÉÍÓÚñáéíóúÑ]{0}?[A-Za-zÁÉÍÓÚñáéíóúÑ\'])?$/
export function validateOnlyNumbers(value) {
  if((!value.match(/[\WA-Za-z]/g)===false) 	|| value.length < 6){
    return true;
  } 
}

export function validateOnlyLetters(value){
  if(!value.match(nombres)) {
    return true
  }
}
